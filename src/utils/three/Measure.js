...
export default class Measure {
  ...
  isCompleted = false; // whether measure operation is completed or not
  mode = '';
  renderer = null;
  scene = null;
  camera = null;
  ...
  raycaster = null;
  mouseMoved = false;
  pointMarkers = []; // used for measure distance and area
  polyline = null; // the line user draws while measuring distance
  faces = null; // the faces user draws while measuring area
  curve = null; // the arc curve to indicate the angle in degree
  tempPointMarker = null; // used to store temporary Points
  tempLine = null; // used to store temporary line, which is useful for drawing line/area/angle as mouse moves
  tempLabel = null; // used to store temporary label as mouse moves
  labels = []; // stores labels, so that we can clear them up when necessary
  pointArray = [];
  lastClickTime = 0; // save the last click time, in order to detect double click event
  settings = Settings;

  constructor(renderer, scene, camera, controls, mode = MeasureMode.Distance, settings = {}) {
    this.mode = mode;
   ...
   * Creates THREE.Line
   */
  createLine(material = Measure.LINE_MATERIAL) {
